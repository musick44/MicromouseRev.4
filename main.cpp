...
#define WALL_SETPOINT_M 1850
#define WALL_THRESHOLD 700
#define ONE_SQUARE 1560
#define ONE_SQUARE_COAST 1550
#define TURN_90_R 510
#define TURN_90_L 545
#define TURN_AROUND 1100
#define NORTH 0x0
#define EAST 0x1
#define SOUTH 0x2
#define WEST 0x3
...
//static struct movementVector forwardCoastMove;
...
static int checkMapCompleteBasic(void);
static void analogRead(void);
static void analogReset(void);
static void resetEnCounts(void);
static void resetErrorArray(void);
static void setMotorMove(struct movementVector, int);
static void stopMotorLeft(void);
static void stopMotorRight(void);
...
//			waitForButton();
		...
		//RESET POSITION IN MAPPING SEQUENCE 10
		while((GPIOB->IDR&0xC0) == 0x80)
		{
			stopMotorLeft();
			stopMotorRight();
			MAP[lastFive[4].xPos][lastFive[4].yPos].walls = 0;
			MAP[lastFive[4].xPos][lastFive[4].yPos].scanned = 0;
			MAP[lastFive[3].xPos][lastFive[3].yPos].walls = 0;
			MAP[lastFive[3].xPos][lastFive[3].yPos].scanned = 0;
			MAP[lastFive[2].xPos][lastFive[2].yPos].walls = 0;
			MAP[lastFive[2].xPos][lastFive[2].yPos].scanned = 0;
			MAP[lastFive[1].xPos][lastFive[1].yPos].walls = 0;
			MAP[lastFive[1].xPos][lastFive[1].yPos].scanned = 0;
			MAP[lastFive[0].xPos][lastFive[0].yPos].walls = 0;
			MAP[lastFive[0].xPos][lastFive[0].yPos].scanned = 0;
			currentXpos = 1;
			currentYpos = 1;
			direction = defaultDir;
			waitForButton();
...
void mapCell(void)
{
	...
	{  
		...
		lastFive[0] = MAP[currentXpos][currentYpos];
		...
		switch(direction) 
		{
			case NORTH:
				...
				if(analog1.middleIRVal>=WALL_THRESHOLD)
				{
					...
					MAP[currentXpos][currentYpos+1].walls|=0x02; //fills in other side of wall
				}
				...
				if(analog1.leftFrontIRVal>=WALL_THRESHOLD) 
				{
					...
					MAP[currentXpos-1][currentYpos].walls|=0x04; //fills in other side of wall
				}
				...
				if(analog1.rightFrontIRVal>=WALL_THRESHOLD) 
				{
					...
					MAP[currentXpos+1][currentYpos].walls|=0x01; //fills in other side of wall
				}
			...
			case WEST:
				...
				{
					MAP[currentXpos][currentYpos].walls|=0x01; //maps wall west
					MAP[currentXpos-1][currentYpos].walls|=0x04;
				}
				//if IR indicates wall to the south, do the thing
				if(analog1.leftFrontIRVal>=WALL_THRESHOLD) 
				{
					MAP[currentXpos][currentYpos].walls|=0x02; //maps wall south
					MAP[currentXpos][currentYpos-1].walls|=0x08;
				}
				//if IR indicates wall to the north, do the thing
				if(analog1.rightFrontIRVal>=WALL_THRESHOLD) 
				{
					MAP[currentXpos][currentYpos].walls|=0x08; //maps wall north
					MAP[currentXpos][currentYpos+1].walls|=0x02;
				}
				MAP[currentXpos][currentYpos].scanned = 1; //marks cell as scanned
				break;
			case SOUTH:
				...
				{
					MAP[currentXpos][currentYpos].walls|=0x04; //maps wall east
					MAP[currentXpos+1][currentYpos].walls|=0x01;
				}
				//if IR indicates wall to the west, do the thing
				if(analog1.rightFrontIRVal>=WALL_THRESHOLD) 
				{
...
Description:  runs loop waiting for the button to be pressed. After the
              button is pressed, it will delay for 1 seconds then return.
Inputs     :  None
Outputs    :  None
...
{
	analog1.leftBackIRVal = 0;
	analog1.leftFrontIRVal = 0;
	analog1.middleIRVal = 0;
	analog1.rightBackIRVal = 0;
	analog1.rightFrontIRVal = 0;
}
			...
			if(handleItMove.moveType == forward)
			{
				if((TIM2->SR&0x1)==0x1)
				{
					...
					if (analog1.leftFrontIRVal > WALL_THRESHOLD)
					{
						...
						error[0] = analog1.leftFrontIRVal - WALL_SETPOINT_L;
						PID = KP*error[0]+(error[9]-error[0])/KD;
					}
					//if right wall
					else if (analog1.rightFrontIRVal > WALL_THRESHOLD)
					{
						...
						error[0] = WALL_SETPOINT_R - analog1.rightFrontIRVal;
						PID = KP*error[0]+(error[9]-error[0])/KD;
					}
					//if no walls
					else
...
//			if((analog1.middleIRVal > WALL_SETPOINT_M)&&(handleItMove.moveType==forward))
//			{
//				if(enCountRight >= enCountLeft)
//				{
//					handleItMove.leftMotorSteps = enCountRight;
//					handleItMove.rightMotorSteps = enCountRight;
//				}
//				if(enCountRight < enCountLeft)
//				{
//					handleItMove.leftMotorSteps = enCountLeft;
//					handleItMove.rightMotorSteps = enCountLeft;
//				}
//			}
			if(handleItMove.rightMotorSteps<=enCountRight)
			{
				//sets right complete flag
				rightMotorFinish = 1;
				
				// turns off timer2 to stop generating PID corrections
				HAL_TIM_Base_Stop(&htim2);
				
				//breaks the right motor
				stopMotorRight();
			}
			if(handleItMove.leftMotorSteps<=enCountLeft)
			{
				//sets left complete flag
				leftMotorFinish = 1;
				
				// turns off timer2 to stop generating PID corrections
				HAL_TIM_Base_Stop(&htim2);
				
				//breaks the left motor
				stopMotorLeft();
			}
		}
		HAL_Delay(50);
...
Status     :  copy pasta, needs testing
***********************************************************************************/
int checkMapCompleteBasic(void)
{
	for(int i = 0;i<MAP_SIZE;i++)
	{
		for(int j = 0;j<MAP_SIZE;j++)
		{
			if(MAP[i][j].scanned == 0)
			{
				return 0;
			}
		}
...
int checkMapComplete(void)
{
	...
	struct map workingCell, targetCell;
	workingCell = resetCell;
	targetCell = MAP[MAP_CENTER][MAP_CENTER];
	bool targetPosFound = 0;
	listStructInit();
	resetFillVals();
	
	//load current position as the starting point and set fillVal
	MAP[1][1].fillVal = 0x8000;
	floodListAdd(MAP[1][1]);
	...
	while(targetPosFound == 0)
	{
		...
		if((workingCell.xPos == targetCell.xPos)&&(workingCell.yPos == targetCell.yPos))
		{
			targetCell.fillVal = workingCell.fillVal;
			targetPosFound = 1;
		}
	...
	for(int i = 0x8000;i<targetCell.fillVal;i++)
	{
		...
		//next cell West
		if((MAP[workingCell.xPos-1][workingCell.yPos].fillVal == (workingCell.fillVal-1))&&((MAP[workingCell.xPos-1][workingCell.yPos].walls&0x0004)==0x0000))
		{
			workingCell = MAP[workingCell.xPos-1][workingCell.yPos];
		}
		//next cell East
		if((MAP[workingCell.xPos+1][workingCell.yPos].fillVal == (workingCell.fillVal-1))&&((MAP[workingCell.xPos+1][workingCell.yPos].walls&0x0001)==0x0000))
		{
			workingCell = MAP[workingCell.xPos+1][workingCell.yPos];
		}
		//next cell South
		if((MAP[workingCell.xPos][workingCell.yPos-1].fillVal == (workingCell.fillVal-1))&&((MAP[workingCell.xPos][workingCell.yPos-1].walls&0x0008)==0x0000))
		{
			workingCell = MAP[workingCell.xPos][workingCell.yPos-1];
		}
		//next cell North
		if((MAP[workingCell.xPos][workingCell.yPos+1].fillVal == (workingCell.fillVal-1))&&((MAP[workingCell.xPos][workingCell.yPos+1].walls&0x0002)==0x0000))
		{
			workingCell = MAP[workingCell.xPos][workingCell.yPos+1];
		}
	}
...
Status     :  seems to work well, will still probably have bugs
***********************************************************************************/
	...
	while(targetPosFound == 0)
	{
		...
		for(int j = 0;j<4;j++)
		{
			...
			if((workingCell.walls&(0x1<<j)) != (0x1<<j))
			{
				...
				{
					case(0):
						if((MAP[workingCell.xPos-1][workingCell.yPos].fillVal == 0)&&(workingCell.xPos-1 <= 16))
						{
							MAP[workingCell.xPos-1][workingCell.yPos].fillVal = (workingCell.fillVal+1);
							floodListAdd(MAP[workingCell.xPos-1][workingCell.yPos]);
						}
					break;
					case(1):
						if((MAP[workingCell.xPos][workingCell.yPos-1].fillVal == 0)&&(workingCell.yPos-1 <= 16))
						{
							MAP[workingCell.xPos][workingCell.yPos-1].fillVal = (workingCell.fillVal+1);
							floodListAdd(MAP[workingCell.xPos][workingCell.yPos-1]);
						}
...
//			if(workingCell.fillVal == targetCell.fillVal)
//			{
//				moveListAdd(forwardCoastMove);
//			}
//			else
//			{
			moveListAdd(forwardMove);
...
Status     :  doesn't turn around after it gets to start
***********************************************************************************/
void genStartVector(void)
{
	...
	while(targetPosFound == 0)
	{
		...
		//if the cell that was loaded has not been scanned, set as destination
		if((workingCell.xPos == 1)&&(workingCell.yPos == 1))
		{
			targetCell.fillVal = workingCell.fillVal;
			targetPosFound = 1;
		}
	...
	//causes a turn around when it gets to the starting position to position the uMouse for a run vector
	moveListAdd(turnAroundMove);
...
void genRunVector(void)
{
	...
	while(targetPosFound == 0)
	{
		...
		//if the cell that was loaded has not been scanned, set as destination
		if((workingCell.xPos == MAP_CENTER)&&(workingCell.yPos == MAP_CENTER))
		{
			targetCell.fillVal = workingCell.fillVal;
			targetPosFound = 1;
		}
...
//	forwardCoastMove.pwmL1 = PWM_PULSE;
//	forwardCoastMove.pwmL2 = BASE_SPEED;
//	forwardCoastMove.pwmR1 = BASE_SPEED-speedTuneR;
//	forwardCoastMove.pwmR2 = PWM_PULSE;
//	forwardCoastMove.leftMotorSteps = ONE_SQUARE_COAST;
//	forwardCoastMove.rightMotorSteps = ONE_SQUARE_COAST;
//	forwardCoastMove.moveType = forward;
	...
	turnRightMove.leftMotorSteps = TURN_90_R;
	turnRightMove.rightMotorSteps = TURN_90_R;
	...
	turnLeftMove.leftMotorSteps = TURN_90_L;
	turnLeftMove.rightMotorSteps = TURN_90_L;
...
/***********************************************************************************
Functions  :  resetErrorArray
Description:  sets the error array to zeros
Inputs     :  None
Outputs    :  None

Status     :  Complete
***********************************************************************************/
void resetErrorArray(void)
{
	for(int i = 0;i < 10;i++)
	{
		error[i] = 0;
